...
program
    ...
    .option("-K, --mfa_token <mfa_token>", "MFA Token key generated by Apigee Edge after enabling multi-factor authentication")
...
function extract_traffic(options) {
  obtain_access_token(options)
    .then(function(access_token_payload) {
      options.access_token = access_token_payload.access_token;
      return get_include_orgs_promise(options)
    })
...
function obtain_access_token(options) {
  //curl -s -d "grant_type=password" --data-urlencode "username=$ae_username2" --data-urlencode "password=$ae_password2"  https://login.apigee.com/oauth/token\?mfa_token\=949630 --header "Content-Type: application/x-www-form-urlencoded;charset=utf-8" -H "accept: application/json;charset=utf-8" -H "authorization: Basic ZWRnZWNsaTplZGdlY2xpc2VjcmV0"
  return request({method: 'POST',
      uri: 'https://login.apigee.com/oauth/token', 
      form: {
        username: options.apigee_mgmt_api_email, 
        password: options.apigee_mgmt_api_password,
        grant_type: 'password'
      },
      qs: {mfa_token: options.mfa_token},
      headers: {
